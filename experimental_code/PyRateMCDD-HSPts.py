...
p.add_argument('-proposal', type=int, help='0: sliding win; 1: *multiplier*', default=0, metavar=0)
p.add_argument('-ws', type=float, help='Win size (sliding win only)', default=0.35, metavar=0.35)
p.add_argument('-out', type=str, help='output tag', default="", metavar="")
...
for iteration in range(n_iterations):	
	...
	if iteration==0:
		...
		if rr<sampling_freqs[0]:
			...
			if use_sliding_win is True:
				Garray_temp= update_parameter_normal_2d_freq((GarrayA[focal_clade,:,:]),G_win_size,m=-MAX_G,M=MAX_G)
			else:
