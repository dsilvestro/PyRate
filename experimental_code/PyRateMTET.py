...
p.add_argument('-overlap',    type=str,   help='table with human overlap', default="", metavar="")
...
def get_death_lik_lineage_time(mu,bl,ex,log_effect):
    eff = np.exp(log_effect)
    mu_overlap = eff * mu
    mu_backgroun = mu
    
    lik =  ex * (1 - ext_while_overlap) * np.log(mu_backgroun) -  \
           (bl * rel_time_trait[:,0] * mu_backgroun) + \
           ex * ext_while_overlap * np.log(mu_overlap) -  \
           (bl * rel_time_trait[:,1] * mu_overlap)
    return lik
...
te_list = data[:,1]
tr_name_list = head[3:]
time_trait_vec = np.random.random(len(ts_list)) + ts_list
...
while iteration <= n_iterations:
    ...
    # lineage-sp rates
    np_sum_rates,prior_Y,prior_Tau,prior_Beta = np.zeros(n_lineages),0,0,0
    for i in range(n_traits):
        ...
        np_sum_rates += transform_trait_multiplier[tr_list_transf[:,i]]
        # calculate priors
