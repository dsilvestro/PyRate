...
for sim_no in range(100):
	...
	R = 50. # root age
	N = np.random.randint(50,150) # no. species
	E = round(np.random.uniform(0,0.25),3) # fraction of extant
	q_rate = round(np.random.uniform(0.5,1.5),3) # preservation rate


	W_shape = round(np.random.uniform(0.5,1.5),3)
	...
	#print len(TE[TE==0]), N, E, len(TE[TE==0])/float(N)
	#print TS[TE==0]
	...
	# names log files
	output_name = "sim_%s_%s_%s_%s_%s_%s" % (N,E,rho,q_rate,W_shape,W_scale)
