...
p.add_argument("-lag_model",  help='lag model', action='store_true', default=False)
p.add_argument("-max_lag",  type=float, help='max lag ', default=10, metavar=10)
...
WARNING: the predictor must have values equally speced in time!
...
def get_time_series(Temp_values, lag=0):
	time_cut = len(times_of_T_change[times_of_T_change<max(ts)])#, len(times_of_T_change)
	times_of_T_change_lag = times_of_T_change + lag
	
	times_of_T_change_lag = times_of_T_change_lag[times_of_T_change_lag>0]
	#times_of_T_change_lag = times_of_T_change_lag[0:time_cut]
	
	#print times_of_T_change
	#print times_of_T_change_lag
	
	# create matrix of all events sorted (1st row) with indexes 0: times_of_T_change, 1: ts, 2: te, 3: te=0
	z=np.zeros(len(te))+2
	z[te==0] = 3
	all_events_temp= np.array([np.concatenate((times_of_T_change_lag,ts,te),axis=0),
	np.concatenate((np.zeros(len(times_of_T_change_lag)),np.zeros(len(ts))+1,z),axis=0)])
	...
	Temp_at_events= get_VarValue_at_time(times_of_T_change_tste,Temp_values,times_of_T_change_indexes,times_of_T_change_lag,max(ts))
	#print len(Temp_at_events)
	...
	if args.verbose:
		...
		for i in range(1,len(all_events)):
			if all_events[i] != all_events[i-1]:
				print np.array([all_events[i],Temp_at_events[i], Dtraj[i,0]])
	...
	return times_of_T_change_lag, Temp_at_events, Dtraj, shift_ind, max_times_of_T_change_tste, all_events, n_time_bins, abs_diff, V_list


times_of_T_change_lag, Temp_at_events, Dtraj, shift_ind, max_times_of_T_change_tste, all_events, n_time_bins, abs_diff, V_list = get_time_series(Temp_values, lag=0)
#a, b, c, d, max_times_of_T_change_tste, d, n_time_bins, e, f = get_time_series(Temp_values, lag=0)
[V1, V2, V3, V4] = V_list

if args.verbose:
	times_of_T_change_lag, Temp_at_events, Dtraj, shift_ind, max_times_of_T_change_tste, all_events, n_time_bins, abs_diff, V_list = get_time_series(Temp_values, lag=-0.5)
	#a, b, c, d, max_times_of_T_change_tste, d, n_time_bins, e, f = get_time_series(Temp_values, lag=0)
	[V1, V2, V3, V4] = V_list
	quit()
...
for iteration in range(mcmc_gen * len(scal_fac_TI)):	
	...
	if lagged_model: lag_time=lag_timeA+0
	if iteration>10:
	...
	if est_start_time:
		...
		updated_lag = 0 
		lag_time = lag_timeA +0.
		if np.random.random() < 0.1:
			lag_time = update_parameter(lag_timeA,m=0,M=args.max_lag,d=w_size_start_time)
			#if 2>1:
			times_of_T_change_lag, Temp_at_events, Dtraj, shift_ind, max_times_of_T_change_tste, all_events, n_time_bins, abs_diff, V_list = get_time_series(Temp_values, lag= -lag_time)
			[V1, V2, V3, V4] = V_list
			updated_lag = 1 
			modified_Temp_at_events = Temp_at_events
		
	if args_mSpEx[0]==0: 
		#print len(modified_Temp_at_events), len(shift_ind)
	...
	if (lik_alter + prior + hasting) - postA >= log(rand.random()) or iteration==0 or GIBBS == 1:
		...
		if lagged_model: 
			lag_timeA = lag_time
			all_par_list = [times_of_T_change_lag, Temp_at_events, Dtraj, shift_ind, max_times_of_T_change_tste, all_events, n_time_bins, abs_diff, V_list]
		if est_start_time: effect_start_timeA=effect_start_time
	if lagged_model:
		if updated_lag:
			[times_of_T_change_lag, Temp_at_events, Dtraj, shift_ind, max_times_of_T_change_tste, all_events, n_time_bins, abs_diff, V_list] = all_par_list
			[V1, V2, V3, V4] = V_list
	...
	if iteration % print_freq ==0: 
		...
		if lagged_model: print "lag:", lag_timeA
	if iteration % sampling_freq ==0:
		...
		if lagged_model:   log_state=[iteration,postA,likA,priorA] + list(lik_pA) + list(l0A) + list(m0A) +g_vec_write +[lag_timeA] + [hypRA,hypGA] + [scal_fac_TI[scal_fac_ind]]
