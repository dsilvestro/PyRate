...
def run_model_testing(Xdata,q_shift=0,min_n_fossils=2,verbose=1):
    ...
    if verbose ==1: print("HPP* max likelihood:", np.round(resHPPm[0],2), "q rate:", abs(resHPPm[1]))
    ...
    if verbose ==1: print("TPP* max likelihood:", np.round(resTPPm[0],2),"q rates:", abs(np.array(resTPPm[1])))
