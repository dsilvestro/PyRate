...
if summary_file != "":
	...
	def get_marginal_rates_plotHR(times,rates,grid):
		mr = np.zeros(len(grid))
		for i in range(len(times)-1):
			try:
				grid_ind = np.intersect1d((grid<=times[i]).nonzero()[0],(grid>=times[i+1]).nonzero()[0])
				rates_mean = np.mean(rates[i])
				mr[grid_ind.astype(int)] = rates_mean
			except: pass
		return mr
	
	
	min_root_age = max(age_vec)
	#if max_age> 0: min_root_age=max_age
	max_death_age= min(age_vec)
	grid_cell_size = 0.1
	n_bins= 420 #int((min_root_age-max_death_age)/grid_cell_size)
	grid = np.round(np.linspace(min_root_age,max_death_age,n_bins),1)
	n_samples = np.shape(marginal_L)[0]
	print np.shape(t)[0]
	m_sp_matrix = np.zeros((n_samples,n_bins))
	m_ex_matrix = np.zeros((n_samples,n_bins))
	print "Extracting marginal rates..."
	
	#print np.mean(marginal_M,axis=0)
	
	m_shift_times = np.array([min_root_age]+ list(age_vec)+[max_death_age])
	for i in range(len(grid)):
		indx_temp = (np.round(m_shift_times,1)==grid[i]).nonzero()[0]
		#print indx, grid[i]
		if len(indx_temp)>0:
			indx = indx_temp[0]
			#print grid[i], indx,marginal_M[1,indx], np.shape(marginal_M)
			m_sp_matrix[:,i] = marginal_L[:,indx]
			m_ex_matrix[:,i] = marginal_M[:,indx]
		else:
			m_sp_matrix[:,i] = marginal_L[:,indx]
			m_ex_matrix[:,i] = marginal_M[:,indx]
			

	#for i in range(1): #range(n_samples):		
	#	#l_shift_times = np.array([min_root_age]+ list(age_vec)+[max_death_age])
	#	#m_sp_matrix[i] = get_marginal_rates_plotHR(l_shift_times,marginal_L.T,grid)
	#	m_shift_times = np.array([min_root_age]+ list(age_vec)+[max_death_age])
	#	m_ex_matrix[i] = get_marginal_rates_plotHR(m_shift_times,marginal_M[i],grid)
	#	print "FUCK", len(marginal_M[0]),len(m_shift_times),len(grid),marginal_M[i]
	#	#print get_marginal_rates_plotHR(m_shift_times,marginal_M,grid)
	#
	#	quit()
	...
	if save_HR_log_file==1:
		...
		out="%s/%s_HR_marginal.log" % (output_wd,name_file)
		logfile = open(out, "w") 
