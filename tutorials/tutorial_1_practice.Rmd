...
-   Argument order afterwards:
    ...
    -   No flag or with `-A 4` for RJMCMC

    -   `-A 2` for BDMCMC

    -   Preservation model flag or lack thereof to default to nHPP

    -   Gamma model flag `-mG`

    -   Specify which randomized replicate (remember, the separate datasets for differently randomized ages): `-j 1` flag, where the "1" in this case indicates the replicate number we want to analyze

    -   MCMC parameters:

        -   `-n 20000000` : do 20 million iterations. Default is 10 million! If the dataset is really large, Iterations may need to be increased in order to reach convergence
...
1.  **sum.txt:** text file with the list of settings used in the analysis
2.  **mcmc.log**: tab-separated table with MCMC samples of:
    1.  Preservation Model: posterior, prior, likelihood (PP_lik)
    2.  Birth-Death Model: posterior, prior, likelihood