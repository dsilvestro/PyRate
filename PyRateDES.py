...
p.add_argument('-varE',      type=str, help='Directory to time variable files for extinction (takes all files)', default="", metavar="")
...
#p.add_argument('-TdVarD',    help='Combine time-dependent dispersal (as in qtimes) with environmental-dependence (varD)', action='store_true', default=False)
#p.add_argument('-TdVarE',    help='Combine time-dependent extinction (as in qtimes) with environmental-dependence (varE)', action='store_true', default=False)
...
if input_data=="":
    ...
    if do_DdE:
        model_tag+= "_DdE"
    if do_varE and args.linE is False:
        model_tag+= "_Eexp"
    if do_varE and args.linE:
        model_tag+= "_linE"
...
update_rate_freq_d = np.maximum(0.1, 1.5/sum(np.size(dis_rate_vec)))
update_rate_freq_e = np.maximum(0.1, 1.5/sum(np.size(ext_rate_vec)))
update_rate_freq_r = np.maximum(0.1, 1.5/sum(np.size(r_vec)))
if verbose == 1:
        ...
        def get_covar_effect(covar, par, de, transf, plotCI = np.array([0.95])):
            ...
            for i in range(len_par):
                if transf == 1: # Time/environment-dependency and trait dependency
                    rate[i,:] = de[i] * np.exp(np.sum(par[i] * covar, axis = 1))
                if transf == 2: # DivdD
...
for i in range(len(time_varD)):
    head+= "\td12_%s" % (ts_rev[i])
for i in range(len(time_varD)):
    head+= "\td21_%s" % (ts_rev[i])
for i in range(len(time_varE)):
    head+= "\te1_%s" % (ts_rev[i])
for i in range(len(time_varE)):
    head+= "\te2_%s" % (ts_rev[i])
head=head.split("\t")
        ...
        if argsG is False:
            for l in list_taxa_index:
                ...
                lik_tmp = lik_DES_taxon([l, w_list, vl_list, vl_inv_list, Q_list, Q_index_temp, delta_t,
                                         r_vec2, rho_at_present_LIST, r_vec_indexes_LIST, sign_list_LIST, OrigTimeIndex, Q_index, bin_last_occ,
            ...
            for l in list_taxa_index:
                ...
                if do_traitS:
                    idx_r_vec2 = [1,2]
                    r_vec2[:,idx_r_vec2] = np.exp(-bin_size * np.exp(np.sum(trait_parS * traitS[l])) * -np.log(r_vec2[:,idx_r_vec2])/bin_size)
#                a = -np.log(r_vec2)/bin_size
#                b = np.tile(a, pp_gamma_ncat).reshape((pp_gamma_ncat, r_vec2.shape[0], 4))
#                c = np.repeat(-bin_size * YangGamma, pp_gamma_ncat * r_vec2.shape[0]).reshape((pp_gamma_ncat, r_vec2.shape[0], 4))
#                r_vec_Gamma_all = np.exp(c * b)
...
def lik_opt(x, grad):
    ...
    if args.TdD and do_varD is False: # time dependent D
    ...
    if args.TdE and do_varE is False:
        ...
        transf_e = 7
        do_approx_div_traj = 1
    elif args.TdE and do_varE: # Time- and environment-dependent extinction
        ext_vec = ext_vec[Q_index,:]
        ext_vec = ext_vec[0:-1]
        transf_e = 8
...
if args.A == 3:
    ...
    div_iter = 1
    div_timeout = 1
    if args.A3set[4] == 1 and any( any(args.TdD is False or (args.TdD and do_varD)) or do_DivdD or any(args.TdE is False or (args.TdE and do_varE)) or do_DivdE or args.DdE or argstraitD != "" or argstraitE != "" or argscatD != "" or argscatE != "" or argstraitS != ""): # or (data_in_area != 0 and argsG) 
...
#        args.A3set[3] = args.A3set[3] / 3
        ...
        if do_varD:
            ...
            if args.lgD: 
                frombound = int(np.max(opt_ind_x0_log_dis)) + 1
        else:
    ...
    if args.A3set[4] == 1 and args.TdE is False and any(argstraitD != "" or argstraitE != "" or argscatD != "" or argscatE != ""):
        ...
        if do_varE:
            ...
            if args.lgE:
                frombound = int(np.max(opt_ind_x0_log_ext)) + 1
        else:
...
for it in range(n_generations * len(scal_fac_TI)):
    ...
    if r[0] < update_freq[0]: # DISPERSAL UPDATES
        ...
        elif args.TdD and do_varD:
            if r[1] < .5: # Update covar
                covar_parD = update_parameter_uni_2d_freq(covar_parD_A, d=0.1*scale_proposal_d, f = f_varD, m = m_d, M = M_d)
            else:
                if equal_d is True:
...
#        x0_logistic[[1,2]] = 0
    ...
    if args.DdE: # Dispersal dep Extinction
        ...
        transf_e = 7
        do_approx_div_traj = 1
    elif args.TdE and do_varE is False: # Time dep Extinction
        ext_vec = ext_rate_vec[Q_index,:]
        ext_vec = ext_vec[0:-1]
        transf_e=0
    elif args.TdE and do_varE:  # Skyline rates plus environmental dependence
        ext_vec = ext_rate_vec[Q_index,:]
        ext_vec = ext_vec[0:-1]
        transf_e = 8
        do_approx_div_traj = 0 # Check later if this works
    ...
    if model_DUO:
        numD12res = rescale_vec_to_range(np.log(1+numD12), r=10., m=0)
        numD21res = rescale_vec_to_range(np.log(1+numD21), r=10., m=0)
    ...
    if np.isfinite((lik_alter+prior+hasting)) == True:
        ...
        if runMCMC == 1:
            ...
            else: rr = np.log(np.random.uniform(0,1))
